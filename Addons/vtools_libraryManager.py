bl_info = {
	...
	"version": (0, 0, 2),
	"blender": (2, 76, 0),
	"location": "View3D > Panel Tools > Texture libraries panel (sculpt mode)",
	"warning": "",
...
def selectImages (p_fileList):
	...
	img_list = [item for item in file_list if item[-3:] == 'png' or item[-3:] == 'jpg' or item[-4:] == 'jpeg' or item[-3:] == 'tga' or item[-3:] == 'psd']
...
class VTOOLS_UIL_textureLibraryUI(bpy.types.UIList):
	...
	def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
		...
		if item.parent == "None":
			...
			if item.loaded == True:
				row.prop(item, "loaded", text="",emboss = False, icon='RESTRICT_VIEW_OFF' )
				
			else:
				row.prop(item, "loaded", text="",emboss = False, icon='RESTRICT_VIEW_ON')
...
class LBM_PN_LibraryManager(bpy.types.Panel):
	...
	def draw(self,context):
		...
		col.operator(VTOOLS_OP_addLibrary.bl_idname, text = "", icon = "ZOOMIN")
		col.operator(VTOOLS_OP_removeLibrary.bl_idname, text = "", icon = "ZOOMOUT")
